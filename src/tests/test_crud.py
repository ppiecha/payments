...
from src.main.crud import create_user, create_transaction, get_wallet, \
    get_user, get_transaction
from src.main.model import UserResponse, DepositRequest, TransferRequest, \
    TransactionType
...
async def test_create_client(sample_user1):
    ...
    user_row = await get_user(first_name=sample_user1.first_name,
                              last_name=sample_user1.last_name)
    wallet = await get_wallet(user_id=user_row['id'])
    assert list(user.dict().items()) == list(
        UserResponse(**sample_user1.dict(), user_id=user_row['id'],
...
async def test_deposit(sample_user1):
    ...
    transaction = await create_transaction(to_user_id=deposit.user_id,
...
async def test_transfer(sample_user1, sample_user2):
    ...
    await create_transaction(to_user_id=deposit.user_id,
                             amount=deposit.amount)
    to_user = await create_user(request=sample_user2)
    transfer = TransferRequest(from_user_id=from_user.user_id,
                               to_user_id=to_user.user_id, amount='20')
    await create_transaction(to_user_id=transfer.to_user_id,
    ...
    from_transaction = await get_transaction(wallet_id=from_wallet['id'],
